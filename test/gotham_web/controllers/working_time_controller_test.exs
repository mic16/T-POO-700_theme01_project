defmodule GothamWeb.WorkingTimeControllerTest do
  ...
  @create_attrs %{
    end: ~N[2021-10-25 09:00:00],
    start: ~N[2021-10-25 09:00:00]
  }
  @update_attrs %{
    end: ~N[2021-10-26 09:00:00],
    start: ~N[2021-10-26 09:00:00]
  }
  ...
  describe "update working_time" do
    ...
    test "renders working_time when data is valid", %{conn: conn, working_time: %WorkingTime{id: id} = working_time} do
      ...
      assert %{
               "id" => ^id,
               "end" => "2021-10-26T09:00:00",
               "start" => "2021-10-26T09:00:00"
             } = json_response(conn, 200)["data"]
    end
